...
class SafeRL_RolloutStorage:
    """
    Rollout storage for constrained RL algorithms.
    ...
    def compute_cost_returns(self, last_cost_values, gamma, lam, normalize_advantage: bool = True):
        ...
        for step in reversed(range(self.num_transitions_per_env)):
            ...
            # Cost Advantage: A_c(s_t, a_t) = delta_c_t + gamma * lambda * A_c(s_{t+1}, a_{t+1})
        ...
        # Normalize the cost advantages if flag is set
        if normalize_advantage:
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        """Override to include cost data in mini-batches."""
        if self.training_type != "rl":
