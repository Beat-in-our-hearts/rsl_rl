...
class SafeRLOnPolicyRunner:
    ...
    def __init__(self, env: SafeRLVecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        if self.alg_cfg["class_name"] == "PPO_Lagrangian":
        ...
        policy: ConstraintActorCritic = policy_class(
        ...
        if "rnd_cfg" in self.alg_cfg and self.alg_cfg["rnd_cfg"] is not None:
            ...
            if rnd_state is None:
                raise ValueError("RND state not found in observations but RND is enabled")
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    obs, rewards, costs, dones, infos = self.env.step(actions.to(self.env.device))
                    # Move to device
                    obs, rewards, costs, dones = (obs.to(self.device), rewards.to(self.device), costs.to(self.device), dones.to(self.device))
                    ...
                    if self.log_dir is not None:
                        ...
                        costbuffer.extend(cur_cost_sum[new_ids][:, 0].cpu().numpy().tolist())  # for safe RL
                        lenbuffer.extend(cur_episode_length[new_ids][:, 0].cpu().numpy().tolist())
                        cur_reward_sum[new_ids] = 0
                        cur_cost_sum[new_ids] = 0  # for safe RL
